...
window.__COMIC_DOWNLOADER_BUILD__ = {"target":"prod","isDev":false,"version":"3.2.1"};
(() => {
  // src/build-constants.js
  var BUILD_INFO = {
    target: true ? "prod" : "prod",
    version: true ? "3.2.1" : "0.0.0",
    isDev: true ? false : false
  };
  ...
  (function() {
    ...
    const originalGMGetValue = GM_getValue2.bind(globalThis);
    const originalGMSetValue = GM_setValue2.bind(globalThis);
    function runtimeGetValue(key, defaultValue) {
      const mapping = RUNTIME_KEY_MAP[key];
      if (mapping) {
        const value = mapping.get();
        return value === void 0 ? defaultValue : value;
      }
      return originalGMGetValue(key, defaultValue);
    }
    function runtimeSetValue(key, value) {
      const mapping = RUNTIME_KEY_MAP[key];
      if (mapping) {
        mapping.set(value);
        return value;
      }
      return originalGMSetValue(key, value);
    }
    const GM_getValue2 = runtimeGetValue;
    const GM_setValue2 = runtimeSetValue;
    ensureRuntimeState();
    ...
    class ChapterSelectorUI {
      ...
      cancelDownload() {
        ...
        GM_setValue2("cancelBatchDownload", true);
        this.isDownloading = false;
      ...
      startProgressSync() {
        ...
        this.progressSyncInterval = setInterval(() => {
          try {
            const currentChapter = GM_getValue2("currentChapterName", "");
            const currentImage = GM_getValue2("currentImage", 0);
            const totalImages = GM_getValue2("totalImages", 0);
            const downloadStatus = GM_getValue2("downloadStatus", "");
            const pdfGenerationComplete = GM_getValue2("pdfGenerationComplete", false);
            const currentPDFPage = GM_getValue2("currentPDFPage", 0);
            const totalPDFPages = GM_getValue2("totalPDFPages", 0);
    ...
    class ComicDownloader {
      constructor() {
        try {
          ...
          this.pdfMode = GM_getValue2("pdfMode", "longpage");
          this.isDownloading = false;
      ...
      updateImageProgress(current, total) {
        GM_setValue2("currentImage", current);
        if (typeof total === "number" && total > 0) {
          GM_setValue2("totalImages", total);
        }
      ...
      updatePdfProgress(current, total) {
        GM_setValue2("currentPDFPage", current);
        GM_setValue2("totalPDFPages", total);
        if (this.ui?.infoText && total > 0) {
      ...
      async waitForImagesAndInit() {
        ...
        const isBatchMode = GM_getValue2("autoDownload", false);
      ...
      async downloadComic() {
        ...
        if (this.adapter.hasMultiplePages()) {
          ...
          GM_setValue2("totalImages", allImageUrls.length);
      ...
      async downloadImagesFromUrls(imageUrls) {
        ...
        GM_setValue2("totalImages", imageUrls.length);
      ...
      async downloadImages(start, end) {
        ...
        GM_setValue2("totalImages", imageElements.length);
      ...
      async generateScrollModePDF(pdf, images, sizes) {
        ...
        for (let i = 0; i < validIndices.length; i++) {
          ...
          if (currentPageHeight + scaledHeight > A4_HEIGHT && currentPageImages.length > 0) {
            ...
            GM_setValue2("currentPDFPage", processedImages);
            GM_setValue2("totalPDFPages", validIndices.length);
      ...
      async generateLongPagePDF(pdf, images, sizes) {
        ...
        for (let pageNum = 0; pageNum < pages.length; pageNum++) {
          ...
          for (let i = 0; i < page.images.length; i++) {
            ...
            GM_setValue2("currentPDFPage", pdfProgress);
            GM_setValue2("totalPDFPages", totalImages);
...
⊗ 已取消: ${cancelledChapters.length}个`);
    ...
    function initialize() {
      ...
      addScrollbarStyles();
      const isBatchDownload = GM_getValue2("isBatchDownload", false);
      const autoDownload = GM_getValue2("autoDownload", false);
      ...
      try {
        ...
        const sessionId = GM_getValue2("sessionId", "");
        ...
        if (autoDownload && sessionId && window.comicDownloader.adapter.isChapterPage() && currentTime - parseInt(sessionId.split("_")[0]) < 3e5) {
          ...
          window.comicDownloader.isScrollMode = GM_getValue2("isScrollMode", false);
